...
public class ProfesseurServiceImp implements InProfesseurServices{
    ...
    public ProfesseurDTO getProfesseurById(Long id) {
        Professeur prof = professeurRepository.findByIdStatut(id).orElseThrow(() -> new ResourceNotFoundException("Professeur","id",id));
    ...
    public ProfesseurDTO addProfesseur(ProfesseurDTO professeurDTO) {
		...
		ListAttribut typeProfAttribut = listAttributRepository.findByIdAndStatut(professeurDTO.getIdTypeProf() , GlobalConstant.STATUT_ACTIF)
				.orElseThrow(()-> new ResourceNotFoundException("Type de professeur", "id", professeurDTO.getIdTypeProf()));
    ...
    public void deleteProfesseur(Long id) {
    	Professeur cls = professeurRepository.findByIdStatut(id).orElseThrow(() -> new ResourceNotFoundException("Professeur", "id", id));
	...
	public ProfesseurDTO updateProfesseur(Long id, ProfesseurDTO professeurDTO) {
    	Professeur professeur = professeurRepository.findByIdStatut(id).orElseThrow(()-> new ResourceNotFoundException("Professeur", "id", id));
    	ListAttribut typeProfAttribut = listAttributRepository.findByIdAndStatut(professeurDTO.getIdTypeProf() , GlobalConstant.STATUT_ACTIF)
